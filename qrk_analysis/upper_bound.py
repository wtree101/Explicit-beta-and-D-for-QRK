...
def smallest_D(
    ...
    ``D_precision`` is retained for callers of the previous continuous-search
    API. Integer bisection makes a sub-unit tolerance unnecessary.
    """
    if D_max < 1:
...
def smallest_continuous_D(
    ...
    """Return a continuous relaxation of the smallest feasible ``D``.

    This search is intended for plotting smooth theoretical boundaries. The
    corresponding implementable subsample size is its ceiling.
    """
    if D_max < 1.0:
        raise ValueError("D_max must be at least 1")
    if D_precision <= 0.0:
    ...
    def evaluate(D: float) -> tuple[tuple[float, float] | None, dict | None]:
        return find_alpha_pair(
            T,
            beta,
            D,
    ...
    ceiling_pair, ceiling_result = evaluate(float(D_max))
    if ceiling_pair is None or ceiling_result is None:
        return {
            "smallest_D": None,
            "hit_ceiling": False,
    ...
    final_pair, final_result = evaluate(high)
    if final_pair is not None and final_result is not None:
        best_pair = final_pair
        best_result = final_result
